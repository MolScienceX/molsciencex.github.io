...
function SearchBox({t,compact=false}:{t:Text;compact?:boolean}){const nav=useNavigate();const [q,setQ]=useState("");const go=(e:FormEvent)=>{e.preventDefault();const value=q.trim();if(!value)return;const exact=findMolecules(value).filter(m=>[m.id,m.cas,m.inchikey,m.inchi].some(x=>x.toLowerCase()===value.toLowerCase()));if(exact.length===1&&(/^(MS-\d+|\d{2,7}-\d{2}-\d|[A-Z]{14}-[A-Z]{10}-[A-Z]|InChI=)/i.test(value))){nav(`/molecule/${exact[0].id}/${exact[0].inchikey}`);return}nav(`/search?q=${encodeURIComponent(value)}`)};return <form className={`searchbox ${compact?"compact":""}`} onSubmit={go}><i/><input value={q} onChange={e=>setQ(e.target.value)} placeholder={t.placeholder} aria-label={t.placeholder}/><button>{compact?"→":t.search}</button></form>}
...
function Results({lang,t}:{lang:Lang;t:Text}){
 const [params]=useSearchParams();const batch=params.get("batch")==="1";const q=params.get("q")||"";const [filters,setFilters]=useState<string[]>([]);const [sort,setSort]=useState("relevance");
 const found=useMemo(()=>{if(!batch)return findMolecules(q);const ids=JSON.parse(sessionStorage.getItem("molscience-batch-ids")||"[]") as string[];return molecules.filter(m=>ids.some(id=>[m.id,m.cas,m.inchikey,String(m.cid)].includes(id)))},[batch,q]);
 const shown=useMemo(()=>{const list=found.filter(m=>filters.every(f=>f==="sources"?m.sources.length>0:m.categories.includes(f as "numeric"|"spectra"|"safety")));return [...list].sort((a,b)=>sort==="sources"?b.sources.length-a.sources.length:sort==="completeness"?(b.categories.length+b.sources.length)-(a.categories.length+a.sources.length):sort==="name"?a.name[lang].localeCompare(b.name[lang]):0)},[found,filters,sort,lang]);
 const filterOptions=[["numeric",lang==="zh"?"有实验物性":"Experimental properties"],["spectra",lang==="zh"?"有谱学数据":"Spectral data"],["safety",lang==="zh"?"有安全信息":"Safety information"],["sources",lang==="zh"?"有明确来源":"Traceable sources"]];
 const toggle=(value:string)=>setFilters(current=>current.includes(value)?current.filter(x=>x!==value):[...current,value]);
 const title=batch?(lang==="zh"?"批量 ID 检索":"Batch ID search"):`“${q}”`;const type=batch?(lang==="zh"?"ID 文件":"ID file"):detectType(q,lang);
 return <main className="results container"><div className="resultsTitle"><div><p className="eyebrow">{t.result}</p><h1>{title}</h1><p className="queryRecognition">{t.identified} <b>{type}</b> · {shown.length} {t.count}</p></div><label className="resultSort"><span>{lang==="zh"?"排序":"Sort"}</span><select value={sort} onChange={e=>setSort(e.target.value)}><option value="relevance">{lang==="zh"?"相关度":"Relevance"}</option><option value="completeness">{lang==="zh"?"数据完整度":"Completeness"}</option><option value="sources">{lang==="zh"?"来源数量":"Source count"}</option><option value="name">{lang==="zh"?"分子名称":"Molecule name"}</option></select></label></div><div className="notice"><b>i</b>{t.demo}</div><div className="resultsGrid"><aside className="filters"><div className="filterHeading"><h2>{lang==="zh"?"筛选结果":"Filter results"}</h2>{filters.length>0&&<button onClick={()=>setFilters([])}>{lang==="zh"?"清除":"Clear"}</button>}</div><div className="filterGroup"><h3>{lang==="zh"?"数据可用性":"Data availability"}</h3>{filterOptions.map(([v,l])=><label key={v}><input type="checkbox" checked={filters.includes(v)} onChange={()=>toggle(v)}/><span>{l}</span><small>{v==="sources"?found.filter(m=>m.sources.length>0).length:found.filter(m=>m.categories.includes(v as "numeric"|"spectra"|"safety")).length}</small></label>)}</div><div className="filterGroup"><h3>{lang==="zh"?"数据来源":"Source type"}</h3><p>{lang==="zh"?"第一版展示数据库级来源":"Database-level sources in Phase 1.0"}</p></div></aside><section className="list">{shown.length?shown.map(m=><ResultCard key={m.id} m={m} lang={lang} t={t} query={q}/>):<div className="empty"><Atom/><h2>{t.none}</h2><p>{t.noneText}</p></div>}</section></div></main>
}
function ResultCard({m,lang,t,query}:{m:Molecule;lang:Lang;t:Text;query:string}){
 const value=query.trim().toLowerCase();
 const reason=m.cas.toLowerCase()===value?"CAS":m.inchikey.toLowerCase()===value?"InChIKey":m.formula.toLowerCase()===value?(lang==="zh"?"分子式":"Formula"):(lang==="zh"?"名称或别名":"Name or alias");
 const fields=[
  {label:"MolScience ID",value:m.id},
  {label:"Formula",value:m.formula},
  {label:"Relative molecular mass",value:`${m.mass} g/mol`},
 ...
 return <article className="resultCard"><Link className="resultStructure" to={`/molecule/${m.id}/${m.inchikey}`}><img src={structureImage(m)} alt={`${m.name[lang]} 2D structure`}/></Link><div><div className="resultName"><span><Link to={`/molecule/${m.id}/${m.inchikey}`}><h2>{m.name[lang]}</h2></Link><small>{m.name.en} · {m.iupac}</small><p className="resultAliases">{m.aliases.slice(0,4).join(" · ")}</p></span><em>{reason} {lang==="zh"?"匹配":"match"}</em></div><div className="resultIdentityGrid">{fields.map(x=><span className={x.wide?"wide":""} key={x.label}><small>{x.label}</small><code title={x.value}>{x.value}</code></span>)}</div><footer><div className="coverage"><span>{m.properties.length} {lang==="zh"?"项实验物性":"experimental properties"}</span><span>{m.spectra.length} {lang==="zh"?"项谱学记录":"spectral records"}</span><span>{m.sources.length} {t.sourceCount}</span></div><Link className="link" to={`/molecule/${m.id}/${m.inchikey}`}>{t.detail} →</Link></footer></div></article>
}

function Detail({lang,t}:{lang:Lang;t:Text}){
 const {id,key}=useParams();
 const m=molecules.find(x=>x.id.toLowerCase()===id?.toLowerCase()||x.inchikey===key);
 ...
 return <main><section className="molHero"><div className="container molHeroGrid"><a className="structure" href={structureImage(m,"large")} target="_blank" rel="noreferrer"><img src={structureImage(m,"large")} alt={`${m.name[lang]} 2D bond-line structure`}/><small>{lang==="zh"?"二维键线式 · 点击放大":"2D bond-line structure · Open large"}</small></a><div><p className="eyebrow">MolScience ID / {m.id}</p><h1>{m.name[lang]}</h1><p className="muted">{m.name.en} · {m.iupac}</p><p>{m.description[lang]}</p><div className="identity"><span><small>Formula</small><code>{m.formula}</code></span><span><small>{lang==="zh"?"相对分子质量":"Relative molecular mass"}</small><code>{m.mass} g/mol</code></span><span><small>CAS RN</small><code>{m.cas}</code></span><span><small>InChIKey</small><code>{m.inchikey}</code></span></div></div></div></section><div className="container detailGrid"><aside className="sideNav">{sections.map(([sectionId,label])=><a className={activeSection===sectionId?"active":""} href={`#${sectionId}`} onClick={()=>setActiveSection(sectionId)} key={sectionId}>{label}</a>)}</aside><div className="detailBody"><Section id="overview" n="01" title={t.overview}><div className="aliases"><small>Aliases</small><div className="plainAliasList">{m.aliases.map(x=><span key={x}>{x}</span>)}</div></div></Section><Section id="identifiers" n="02" title={t.identifiers}><div className="idTable"><IdRow label="MolScience ID" value={m.id} t={t}/><IdRow label="Canonical SMILES" value={m.smiles} t={t}/><IdRow label="InChI" value={m.inchi} t={t}/><IdRow label="InChIKey" value={m.inchikey} t={t}/></div></Section><Section id="properties" n="03" title={t.properties}><div className="propertyList">{m.properties.map(p=><article className={`propertyRow ${expanded.includes(p.key)?"isExpanded":""}`} key={p.key}><div className="propertySummary"><div><span>{p.label[lang]}</span><small>{t.recommended}</small></div><strong>{p.value}</strong><p>{p.condition}</p><button onClick={()=>toggle(p.key)} aria-expanded={expanded.includes(p.key)}>{expanded.includes(p.key)?(lang==="zh"?"收起记录":"Hide records"):`${t.allValues} ${p.records.length} ${t.records}`} <b>{expanded.includes(p.key)?"↑":"↓"}</b></button></div>{expanded.includes(p.key)&&<PropertyRecords property={p} lang={lang} t={t}/>}</article>)}</div></Section><Section id="spectra" n="04" title={t.spectroscopy}><div className="spectra">{m.spectra.map(x=><article key={x}><SpectrumSlot lang={lang}/><div><h3>{x}</h3><p><i/>{lang==="zh"?"等待后端谱图数据":"Spectrum data API pending"}</p></div><b>→</b></article>)}</div></Section><Section id="safety" n="05" title={t.safetyInfo}><div className={`safety ${m.safety.level}`}><strong><span>!</span>{m.safety.label[lang]}</strong><ul>{m.safety[lang].map(x=><li key={x}>{x}</li>)}</ul></div></Section><Section id="sources" n="06" title={t.sources}><div className="sourceList">{m.sources.map((x,i)=><article key={x}><span>0{i+1}</span><b>{x}</b><small>Database source</small></article>)}</div><p className="sourceNote">{t.sourceNote}</p></Section></div></div></main>;
}
