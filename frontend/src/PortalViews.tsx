...
export function PortalHome({lang}:{lang:Lang}){
 ...
 return <main>
  ...
  <section className="recentData"><div className="container"><div className="homeSectionHead"><div><p className="portalKicker">RECENT DATA</p><h2>{lang==="zh"?"最近新增数据":"Recently added data"}</h2></div><p>{lang==="zh"?"当前展示可浏览的预览记录；接入数据服务后将按审核入库时间自动更新。":"Preview records are shown for now; verified ingestion times will drive this list after the data service is connected."}</p></div><div className="recentDataList">{molecules.map(m=><Link className="recentDataRow" key={m.id} to={`/molecule/${m.id}/${m.inchikey}`}><span className="recentStatus">{lang==="zh"?"预览数据":"Preview"}</span><span className="recentIdentity"><b>{m.name[lang]}</b><small>{m.iupac}</small><small className="recentMobileMeta">{m.formula} · {m.cas}</small></span><code>{m.formula}</code><code>{m.cas}</code><span className="recentCoverage">{m.categories.map(category=><small key={category}>{category==="numeric"?(lang==="zh"?"实验物性":"Properties"):category==="spectra"?(lang==="zh"?"谱学":"Spectra"):(lang==="zh"?"安全":"Safety")}</small>)}</span><span className="recentSources">{m.sources.length} {lang==="zh"?"个来源":"sources"}</span><b className="rowArrow">→</b></Link>)}</div></div></section>
