...
首页不以“今日新增”作为主要可信度信息。数据总量必须来自真实统计接口，不得使用虚构数字。后端统计接口未就绪时，数据总览整段隐藏，不展示占位符。

Phase 1.0 首页暂不展示本周前沿、按数据类别浏览、常用分子和数据处理流程。这些内容只有在拥有真实数据、明确用途和持续维护能力后再重新评估。

首页下部保留平台、团队、使用指南和贡献指南入口，统一跳转到平台介绍页的对应章节。
